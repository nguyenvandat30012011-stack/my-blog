...
function getPostsByResource(resourceSlug: string) {
  ...
  return allPosts.filter((post) => {
    const postResourceSlug = removeVietnameseTones(post.resourceType);
    return postResourceSlug === resourceSlug.toLowerCase();
  });
}

export default async function ResourcePage({ params }: { params: { slug: string } }) {
  const resolvedParams = await params;
  const posts = getPostsByResource(resolvedParams.slug);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <h1 className="text-2xl font-bold mb-6 capitalize">
          📦 Tài nguyên: {resolvedParams.slug.replace(/-/g, ' ')}
        </h1>
        ...
        {posts.length > 0 ? (
          ...
          <p className="text-slate-500 italic">Chưa có bài viết nào trong mục tài nguyên này.</p>
        )}
      </main>
    </div>
  );
